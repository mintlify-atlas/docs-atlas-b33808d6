...
The push URL is read from `Settings.Get<String>("url", "web")`. The initial push is fired immediately on server start (the timer is pre-subtracted by 20 minutes so the first tick fires at `time 0 + 1,200,000 ms`).
...
- **Maximum 3 connections per external IP** — a fourth connection attempt from the same IP is rejected with `RejectedMsg.TooManyClients`.
