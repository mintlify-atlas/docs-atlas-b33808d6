---
title: "Get Started with sb0t Ares Chat Server"
...
<Steps>
  ...
  <Step title="Launch the application">
    Double-click `sb0t.exe`. The WPF main window opens. On first run, sb0t calls `DoOnce.Run()`, which:

    - Creates the registry key `HKCU\Software\sb0t\sb0t.exe` if it does not already exist.
    - Writes default values for every setting that has not been set yet (for example, port `54321` and room name `my chatroom`).
    - Creates the data directory `%AppData%\sb0t\<app-name>\` and writes the default web chat template to `%AppData%\sb0t\<app-name>\Style\template.htm`.
  ...
  <Step title="Click Start server">
    ...
    1. Calls `Settings.Reset()` to reload all registry values and ensure the data directory exists.
    ...
    5. Starts the server background thread, which loads bans, initialises the captcha manager, starts flood control, and fires `Events.ServerStarted()`.
...
On the very first launch (or whenever `do_once` is `0` in the registry), `DoOnce.Run()` seeds the data directory structure and writes a fresh `template.htm` for the ib0t web chat interface. If the template already exists and has become corrupted, it is backed up as `template.broken` before being replaced.

The **owner password** field is auto-generated as a random 20-character lowercase string on first run and stored in the `owner` registry key. Record this value — it is the password used to claim Host-level access in the room via the Ares client.
...
|---|---|
| `serverlog.txt` | Timestamped server event log |
