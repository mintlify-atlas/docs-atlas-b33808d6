---
title: "Anti-Flood and Rate Limiting in sb0t"
sidebarTitle: "Anti-Flood"
description: "How sb0t's anti-flood system detects and handles message flooding, connection flood protection, and per-user rate limiting."
---
...
The TCP listener in `core/ServerCore.cs` maintains a dictionary (`_connectFlood`) that maps IP address strings to a `(timestamp, count)` tuple. Every time the server accepts a new socket it increments the counter for that IP:
...
If the same IP has already connected **3 or more times** within the active window, the socket is immediately disposed without creating a user object:
...
Stale entries are pruned from `_connectFlood` in the 60-second reset loop (see below). The expiry window is calculated as `Time.Now + 300000` (5 minutes in milliseconds).
