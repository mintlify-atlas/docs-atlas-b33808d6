---
title: "User Levels and Admin Accounts in sb0t"
sidebarTitle: "User Levels"
description: "Understand sb0t's four user levels (Regular, Moderator, Administrator, Host) and how to set passwords for elevated access."
---
...
A user registers by sending the `/register <password>` command. The password must contain at least one letter and one digit:
...
Newly registered accounts receive `ILevel.Regular`. An Administrator or Host must then promote the account through the GUI or a script.

### Promoting an Account

After a user has registered and is online, an Administrator calls `AccountManager.UpdateAccount(admin, client)`. This writes the user's current `client.Level` back to the database so the elevated level persists across reconnections.
